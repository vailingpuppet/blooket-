...
axios.get("https://api.blooket.com/api/users").then(async ({ data: { name, unlocks } }) => {
    let blooks = Object.entries(unlocks).filter(x => x[1] > 1);
      for (var [blook, amount] of blooks) await axios.put("https://api.blooket.com/api/users/sellblook", { name, blook, numSold: amount - 1 });
